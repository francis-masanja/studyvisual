...
const StudyVisualizer = () => {
  ...
  useEffect(() => {
    const fetchMaterial = async () => {
      try {
        const url = user ? `/api/material?id=${id}&username=${user.username}` : `/api/material?id=${id}`;
        const res = await fetch(url);
        const data = await res.json();
        if (data.material) {
          setMaterial(data.material.content_json);
          setInitialProgress(data.material.completion_percentage || 0);
        }
      } catch (error) {
        ...
        {material.type === 'mixed' ? (
          <Tabs.Root defaultValue="document" className="flex-1 flex flex-col">
            ...
            <Tabs.Content value="document" className="flex-1">
              <DocumentView sections={material.sections!} onProgressUpdate={handleProgressUpdate} />
            </Tabs.Content>
...
const FlashcardView = ({ cards, initialProgress = 0, onProgressUpdate }: { cards: StudyFlashcard[], initialProgress?: number, onProgressUpdate: (percentage: number) => void }) => {
  const startingIndex = Math.min(cards.length - 1, Math.max(0, Math.floor((initialProgress / 100) * cards.length)));
  const [currentIndex, setCurrentIndex] = useState(startingIndex);
  const [isFlipped, setIsFlipped] = useState(false);
  const [direction, setDirection] = useState(0);
  const [maxIndexReached, setMaxIndexReached] = useState(startingIndex);
  ...
  const nextCard = () => {
    if (currentIndex < cards.length - 1) {
      ...
      if (nextIndex > maxIndexReached) {
        setMaxIndexReached(nextIndex);
        const percentage = ((nextIndex + 1) / cards.length) * 100;
        onProgressUpdate(percentage);
      }
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col items-center p-4 md:p-10 gap-6 max-w-full overflow-hidden">
      ...
      {/* Card Container */}
      <div className="relative w-full max-w-2xl min-h-[350px] md:min-h-[450px] perspective-1000">
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            ...
            >
              {/* Question Side */}
              <div className="absolute inset-0 backface-hidden bg-cozy-card rounded-[2.5rem] p-8 md:p-12 flex flex-col items-center shadow-xl border border-cozy-secondary/20 overflow-y-auto">
                ...
                <div className="flex-1 flex flex-col justify-center w-full">
                  <p className="text-xl md:text-3xl font-bold leading-tight text-cozy-text text-center">{card.question}</p>
                </div>
                
                <div className="mt-auto pt-8 w-full text-center shrink-0">
                  <p className="text-xs text-cozy-muted italic bg-cozy-accent/30 py-2 rounded-full inline-block px-6">Click to reveal answer</p>
                </div>
              </div>

              {/* Answer Side */}
              <div className="absolute inset-0 backface-hidden bg-cozy-primary rounded-[2.5rem] p-8 md:p-12 flex flex-col items-center shadow-xl border border-cozy-primary rotate-y-180 text-white overflow-y-auto">
                <div className="w-full text-left mb-6 shrink-0">
                  <span className="text-xs font-bold uppercase tracking-widest opacity-70">Note / Explanation</span>
                </div>
                
                <div className="flex-1 flex flex-col justify-center w-full">
                   <p className="text-xl md:text-2xl font-medium leading-relaxed text-center">{card.answer}</p>
                </div>
...
const DocumentView = ({ sections, onProgressUpdate }: { sections: StudySection[], onProgressUpdate: (percentage: number) => void }) => {
  ...
  const handleSectionInView = (idx: number) => {
    if (!viewedSections.has(idx)) {
      const nextViewed = new Set(viewedSections);
      nextViewed.add(idx);
      setViewedSections(nextViewed);
      const percentage = (nextViewed.size / sections.length) * 100;
      onProgressUpdate(percentage);
    }
  };
    ...
    <div className="max-w-3xl mx-auto py-12 px-6 space-y-12">
      {sections.map((section, idx) => (
        <motion.section 
          ...
          onViewportEnter={() => handleSectionInView(idx)}
          viewport={{ once: true }}
